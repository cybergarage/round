...
BOOST_AUTO_TEST_CASE(RegistryManager)
{
  ...
  for (int n=0; n<ROUND_TEST_MAP_SIZE; n++) {
    ...
    BOOST_CHECK_EQUAL(round_registry_getvalue(reg[n]), round_registry_getvalue(round_registry_manager_get(mgr, key)));
  }
  ...
  for (int n=0; n<ROUND_TEST_MAP_SIZE; n++) {
    ...
    BOOST_CHECK_EQUAL((ROUND_TEST_MAP_SIZE-(n+1)), round_registry_manager_size(mgr));
  }
