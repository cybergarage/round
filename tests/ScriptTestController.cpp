...
void Round::Test::ScriptTestController::runScriptStatusTest(RoundLocalNode* node)
{
  ...
  BOOST_CHECK(cluster);
  size_t nodeSize = round_cluster_size(cluster);
  ...
  BOOST_CHECK(round_json_rpc_getresultstring(resObj, &result));
  BOOST_CHECK(result);
  BOOST_CHECK_EQUAL(nodeSize, round_str2int(result));
  
  // Get Cluster Count
  
  size_t clusterSize = round_local_node_getclustersize(node);
  
  BOOST_CHECK(round_local_node_poststringmessage(node, Round::Test::CreateJsonRpcRequestString(RPC_METHOD_GET_CLUSTER_SIZE, ""), &resObj, err));
  BOOST_CHECK(round_json_rpc_getresultstring(resObj, &result));
  BOOST_CHECK(result);
