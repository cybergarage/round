...
bool round_json_map_getbool(RoundJSONObject *obj, const char *key,
  ...
  keyJson = round_json_map_getobject(obj, key);
  if (!keyJson || ((json_typeof(keyJson) != JSON_TRUE) &&
                   (json_typeof(keyJson) != JSON_FALSE)))
    return false;
...
bool round_json_map_setobjectpointer(RoundJSONObject *obj, const char *key,
  ...
  return (json_object_set(obj->jsonObj, key, valObj->jsonObj) == 0) ? true
                                                                    : false;
#else
  return false;
#endif
...
bool round_json_map_setobject(RoundJSONObject *obj, const char *key,
  ...
  isAdded =
  (json_object_set(obj->jsonObj, key, valObj->jsonObj) == 0) ? true : false;
