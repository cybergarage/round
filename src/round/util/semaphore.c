...
bool round_semaphore_init(RoundSemaphore *sem, size_t maxCnt)
{
  ...
  sem->isInitialized = (semaphore_create(mach_task_self(), &sem->semId, SYNC_POLICY_FIFO, (int)maxCnt) == KERN_SUCCESS) ? true : false;
#else
...
#if defined(__APPLE__)
  sem->isInitialized = (semaphore_destroy(mach_task_self(), sem->semId) == KERN_SUCCESS) ? false : true;
#else
...
bool round_semaphore_timedwait(RoundSemaphore *sem, time_t timeoutSec)
{
  ...
  if (0 < timeoutSec) {
    ...
    isSuccess = (semaphore_timedwait(sem->semId, machTimeout) == KERN_SUCCESS) ? true : false;
  }
  else {
