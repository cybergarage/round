...
void round_rpc_server_postresponse(
mUpnpHttpRequest *httpReq, RoundJSONObject *jsonObj) {
...
void round_rpc_server_posterrorresponse(
mUpnpHttpRequest *httpReq, int rpcErrCode) {
  mUpnpHttpResponse *httpRes = mupnp_http_response_new();
...
void round_rpc_server_jsonrpcrequestrecieved(mUpnpHttpRequest *httpReq) {
  ...
  if (jsonContent) {
    ...
    httpReq, ROUND_RPC_ERROR_CODE_INVALID_REQUEST);
    return;
  }
  ...
  if (!upnpDev) {
    round_rpc_server_posterrorresponse(
    httpReq, ROUND_RPC_ERROR_CODE_INTERNAL_ERROR);
    return;
  }
