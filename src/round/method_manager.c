...
bool round_method_manager_addmethod(
RoundMethodManager *mgr, RoundMethod *method) {
  if (!mgr)
    return false;
...
RoundMethod *round_method_manager_getmethod(
RoundMethodManager *mgr, const char *name) {
  if (!mgr)
    return NULL;
...
bool round_method_manager_removemethod(
RoundMethodManager *mgr, const char *name) {
  if (!mgr)
    return false;
...
bool round_method_manager_addengine(
RoundMethodManager *mgr, RoundScriptEngine *engine) {
  if (!mgr)
    return false;
  ...
  round_script_engine_map_remove(
  mgr->engineMap, round_script_engine_getlanguage(engine));
...
RoundScriptEngine *round_method_manager_getengine(
RoundMethodManager *mgr, const char *name) {
  if (!mgr)
    return NULL;
...
bool round_method_manager_removeengine(
RoundMethodManager *mgr, const char *name) {
  if (!mgr)
    return false;
